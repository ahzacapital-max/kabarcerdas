    ...
    <div className="article-layout">
      ...
      <article>
        ...
        <div style={{ display: 'flex', gap: '.75rem', margin: '1.5rem 0', alignItems: 'center' }}>
          ...
          <a href={`https://wa.me/?text=${encodeURIComponent(article.title + ' https://kabarcerdas.my.id/artikel/' + article.slug)}`} target="_blank" rel="noopener noreferrer" title="WhatsApp" style={{ display: 'flex' }}>
          ...
          <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent('https://kabarcerdas.my.id/artikel/' + article.slug)}`} target="_blank" rel="noopener noreferrer" title="Facebook" style={{ display: 'flex' }}>
          ...
          <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent('https://kabarcerdas.my.id/artikel/' + article.slug)}`} target="_blank" rel="noopener noreferrer" title="X" style={{ display: 'flex' }}>
          ...
          <a href={`https://t.me/share/url?url=${encodeURIComponent('https://kabarcerdas.my.id/artikel/' + article.slug)}&text=${encodeURIComponent(article.title)}`} target="_blank" rel="noopener noreferrer" title="Telegram" style={{ display: 'flex' }}>
          ...
          <a href={`https://www.threads.net/intent/post?text=${encodeURIComponent(article.title + ' https://kabarcerdas.my.id/artikel/' + article.slug)}`} target="_blank" rel="noopener noreferrer" title="Threads" style={{ display: 'flex' }}>
      ...
      {others.length > 0 && (
        <div className="article-related">
          ...
          {others.map((a) => (
            <Link key={a.id} href={`/artikel/${a.slug}`} className="related-item">
              <div className="related-cat">{a.category}</div>
              <div className="related-title">{a.title}</div>
              <div className="related-time">{formatRelativeTime(a.published_at)}</div>
            </Link>
          ))}
        </div>
      )}
